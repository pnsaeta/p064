...
The residue is the coefficient of the $$(z - z_0)^{-1}$$ term in the [Laurent expansion](FO-ComplexVariables.md#laurent-expansion). There are several ways to compute residues. The Laurent expansion of an $$n$$th-order pole at $$z_0$$ has the form
\begin{equation}
...
\end{equation}
It may be helpful to use l'Hôpital’s rule in evaluating the limit.
...
\begin{equation}
  \boxed{ \lim_{z \to z_0} \frac{1}{(n-1)!} \dv[n-1]{}{z}[f(z) (z-z_0)^n] = a_{-1} }
\end{equation}
