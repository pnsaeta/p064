...
\end{equation}
where $$\tau$$ is the sampling period. Signals at frequencies greater than the Nyquist frequency are **aliased** to frequencies at the Nyquist frequency or lower; they fold back into the lower-frequency region. Aliasing can cause major headaches when your signal has high-frequency components that exceed the Nyquist frequency.
