...
4. If material properties such as $$c$$ and $$\kappa$$ were functions of temperature (not an uncommon situation), how would you have to modify the Crank-Nicolson matrices shown in Eq.&nbsp;(\ref{eq:CNmatrices}) to account for these temperature dependencies? Is the problem even well-posed?